...
JsonValue = Union['JsonObject', 'JsonArray', int, float, bool, NoneType]
JsonObject = dict[str, JsonValue]
JsonArray = list[JsonValue]


def parser(_input: str) -> JsonValue:
...
class Parser:
    ...
    def parse(self) -> JsonValue:
        token = self.advance()
        return self.parser_from_token(token)

    def parser_from_token(self, token: Token) -> JsonValue:
    ...
    def parse_object(self) -> JsonObject:
        ...
        while key_token.token_type != TokenType.RIGHT_BRACE:
            ...
            json_object[key_token.value] = self.parser_from_token(value_token)
    ...
    def parse_array(self) -> JsonArray:
        ...
        while token.token_type != TokenType.RIGHT_BRACKET:
            ...
            json_array.append(self.parser_from_token(token))
    ...
    def consume(self, token_type: TokenType, error):
