...
class Scanner:
    ...
    def add_string(self):
        ...
        if self.is_at_end():
            raise ValueError("Unexpected string", self.line)
    ...
    def add_number(self):
        ...
        if self.peek() == ".":
            if not self.peek_next().isdigit():
                raise ValueError("Expected digit after . (trying to parser float)")
            ...
            self.tokens.append(Token(TokenType.NUMBER, int(self.source[self.start:self.current])))
